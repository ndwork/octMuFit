...
%     ynp1_1 = max(ynp1_1,-1);
    ...
    prox = prox2NormVectorized(z2/sigma,gammaArr/sigma);
    ynp1_2 = z2 - sigma*prox;
        
%     ynp1_2Check = gamma*projectOntoDualIsoNormUnitBall(z2/gamma);
    ...
    nrms = sqrt(Gxnp1(:,:,1).^2 + Gxnp1(:,:,2).^2);
    weightedNorms = gammaArr.*nrms;
%     cost = sum(sum(abs(Axnp1-b))) + sum(weightedNorms(:));
%     cost = sum(sum(abs(Axnp1-b))) + gamma*evalIsoNorm(Gxnp1);
    cost = .5*sum(sum((Axnp1-b).^2)) + sum(weightedNorms(:));
    costs = [costs,cost];    
