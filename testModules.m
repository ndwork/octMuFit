...
function testModules
  ...
  testApplyA_adjA(I, mask, dz, z, z0, zR );


  % Test applyD and applyAdjointD
  testApplyD_adjD(I, mask, dz);  
  
  % Test applyK and applyAdjointK
  testApplyK_adjK(I, mask, z, dz, dx, z0, zR);


  % Test applyA and applyAdjointA in 1D
  ...
  testMakeA_1D(I, mask, dz, z, z0, zR);
  
  % Compare makeK_1D to applyK_1D
  testMakeK_1D(I, mask, dz, z, z0, zR);
  
  % Compare applyAdjointK1D to makeK' *x
  testApplyAdjointK1D(I, mask, z, dz, z0, zR);

  % Compare Agamma to applyA
  testAGam(I, mask, z, dz, z0, zR);
  
  % Compare A part of objective function computation
  testAGamMinusB(I, mask, z, dz, z0, zR)
...
function testAGam( I2D, mask, z, dz, z0, zR )
  I = I2D(:,1);
  tmp = (z-z0)/zR;
  g = 1 ./ sqrt( tmp.^2 + 1 );
  gam = rand(size(I));
  ...
  applyAgam = applyA(gam, mask, I, dz, z, z0, zR);
...
function testAGamMinusB(I2D, mask, z, dz, z0, zR)
  I = I2D(:,1);
  b = makeb(I);
  tmp = (z-z0)/zR;
  g = 1 ./ sqrt( tmp.^2 + 1 );
  ...
  Agam = A*gam;
  matrixEquiv = (1/2)*norm(Agam - b).^2;

  relDiff = norm(objFuncPart - matrixEquiv) / matrixEquiv;
  disp(['Difference between \|A\gamma - b\|^2 and matrix equiv:' num2str(relDiff)]);

  [Agamma,Dgamma] = applyK1D(gam,mask,I,z,dz,z0,zR);
  matrixEquiv2 = .5*sum((Agamma - b).^2); % + eta*sum(abs(Dgamma));
  relDiff = norm(objFuncPart - matrixEquiv2) / matrixEquiv2;
  disp(['Difference between \|A\gamma - b\|^2 and matrix equiv2:' num2str(relDiff)]);
...
function testObjFunc(I, mask, z, dz, z0, zR)
  ...
  objF = objFunction(gamma, I1D, dz, g, D, eta);
  
  objF_matrx = (1/2)*norm(A*gamma - b).^2 + eta*norm(D*gamma, 1);
...
function testApplyAdjointK1D(I, mask, z, dz, z0, zR)
  I1D = I(:,1);
  x1 = rand(size(I1D));
  x2 = rand(size(I1D));
  x = [x1; x2];
  ...
  tmp = (z-z0)/zR;
  g = 1 ./ sqrt( tmp.^2 + 1 );
  K = makeK_1D(I1D, dz, g, mask);
  adjKx = K'*x;
  
  appAdjKx = applyAdjointK1D( x1, x2, I1D, mask, z, dz, z0, zR );
...
function testMakeK_1D(I, mask, dz, z, z0, zR)
  I1D = I(:,1);
  x = rand(size(I1D));
  ...
  [appKx1, appKx2] = applyK1D(x, mask, I1D, z, dz, z0, zR);
  appKx = [appKx1; appKx2];
  diff = norm(appKx - Kx);
  disp(['Difference between K*x and applyK_1D to x: ' num2str(diff)]);  
end

function testMakeA_1D(I, mask, dz, z, z0, zR)
  I1D = I(:,1);
  x = rand(size(I1D));
  tmp = (z-z0)/zR;
  g = 1 ./ sqrt( tmp.^2 + 1 );
  ...
  appAx = applyA(x, mask, I1D, dz, z, z0, zR);
  
  diff = norm(appAx - Ax);
  disp(['Difference between A*x and applyA_1D to x: ' num2str(diff)]);  
end

function testApplyA_adjA(I, mask, dz, z, z0, zR )
  % check to see if <A*x,y> = <x,adjA*y>
  x = rand(size(I));
  y = rand(size(I));
  Ax = applyA( x, mask, I, dz, z, z0, zR );
  Axy = Ax .* y;
  innerProd1 = sum( Axy(:) );
  adjAy = applyAdjointA( y, mask, I, z, dz, z0, zR );
  xAdjAy = x .* adjAy;
  innerProd2 = sum( xAdjAy(:) );
  errorA = abs( innerProd1 - innerProd2 );
  disp(['Adjoint A error: ', num2str(errorA) ]);
end
function testApplyD_adjD(I, mask, dz)
  dim = 1;
  x = rand( size(I) );
  y = rand( size(I) );
  Dx = applyD( x, mask, dz, dim);
  adjDy = applyAdjointD(y, mask, dz, dim);
  Dxy = Dx .* y;
  adjDyx = adjDy .* x;
  innerProd1 = sum( Dxy(:) );
...
function testApplyK_adjK(I, mask, z, dz, dx, z0, zR)
  y1 = rand( size(I) );
  y2 = rand( size(I) );
  y3 = rand( size(I) );
  x = rand ( size(I) );
  
  [Kx1, Kx2, Kx3] = applyK( x, mask, I, z, dz, dx, z0, zR );
  tmp1 = Kx1.*y1 + Kx2.*y2 + Kx3.*y3;
  innerProd1 = sum( tmp1(:) );
  adjKy = applyAdjointK( y1, y2, y3, I, mask, z, dz, dx, z0, zR );
  tmp2 = x .* adjKy;
  innerProd2 = sum( tmp2(:) );
...
function testK_adjointK_1D(I, dz, z, z0, zR)
  ...
  x1D = rand(size(I1D));
  Ax = applyA(x1D,mask1D,I1D, dz, z, z0, zR );
  innerProd1 = sum( Ax .* y1 );
  adjAy = applyAdjointA(y1,mask1D,I1D,z,dz,z0,zR);
  innerProd2 = sum( x1D .* adjAy );
  errorK = abs( innerProd1 - innerProd2 );
  disp(['Adjoint A1D error: ', num2str(errorK) ]);
  
  % Test applyD and applyAdjointD in 1D
  Dx = applyD(x1D,mask1D,dz,1 );
  innerProd1 = sum( Dx .* y1 );
  adjDy = applyAdjointD(y1,mask1D,dz,1);
  ...
  y2 = rand(size(I1D));
  [Kx1, Kx2] = applyK1D( x1D, mask1D, I1D, z, dz, z0, zR );
  tmp1 =  Kx1.*y1 + Kx2.*y2;
  innerProd1 = sum( tmp1(:) );
  adjKy = applyAdjointK1D( y1, y2, I1D, mask1D, z, dz, z0, zR );
  tmp2 = x1D.*adjKy;
  innerProd2 = sum( tmp2(:) );
  errorK = abs( innerProd1 - innerProd2 );
...
function testProxF_1D( I )
  ...
  [prxF1, prxF2] = proxF1D(x1, x2, b, lambda, eta);
